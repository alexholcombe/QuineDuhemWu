...
#dg<- mutate(dj, new_ph = ph * updateFactor)

dg$ph <- factor(dg$ph)
g<-ggplot(dj,aes(x=x,y=newh)) + geom_line() +theme_apa() + 
...
g<-g+ylab(    TeX('$b*\\frac{ 1-b }{ 1-b*b$')   ) + ggtitle('Updated probability for two identical-prior hypotheses after experiment failure')
plot(g)
```
